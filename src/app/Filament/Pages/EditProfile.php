...
class EditProfile extends BaseEditProfile
{
    ...
    public static function logoutOtherBrowserSessions($password): void
    {
        if (! Hash::check($password, Auth::user()->password)) {
            Notification::make()
                ->danger()
                ->title(('filament-edit-profile::default.incorrect_password'))
                ->send();
        ...
        Notification::make()
            ->success()
            ->title(('filament-edit-profile::default.browser_sessions_logout_success_notification'))
            ->send();
    }
                    ...
                    ->schema([
                        ...
                        Forms\Components\Actions::make([
                            Forms\Components\Actions\Action::make('deleteBrowserSessions')
                                ->label(('filament-edit-profile::default.browser_sessions_log_out'))
                                ->requiresConfirmation()
                                ->modalHeading(('filament-edit-profile::default.browser_sessions_log_out'))
                                ->modalDescription(('filament-edit-profile::default.browser_sessions_confirm_pass'))
                                ->modalSubmitActionLabel(('filament-edit-profile::default.browser_sessions_log_out'))
                                ->form([
                                    Forms\Components\TextInput::make('password')
                                        ->password()
                                        ->revealable()
                                        ->label(('filament-edit-profile::default.password'))
                                        ->required(),
                                ])
